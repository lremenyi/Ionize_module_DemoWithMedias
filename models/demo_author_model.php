...
class Demo_author_model extends Base_model
{
	...
	public function __construct()
	{
		...
		$this->set_pk_name('id_module_demo_author');
	...
	public function get_linked_author($parent, $id_parent)
	{
		...
		$query = $this->{$this->db_group}
			...
			->join(
				$this->_author_table,
				$this->_author_table.'.id_module_demo_author = ' . $this->_link_table.'.id_module_demo_author',
				'left'
			)
			->join(
				$this->_author_lang_table,
				$this->_author_lang_table.'.id_module_demo_author = ' . $this->_author_table.'.id_module_demo_author',
				'left'
			)
	 ...
	 *
	 */
	public function link_author_to_parent($parent, $id_parent, $id_module_demo_author)
	{
		$data = array(
			'parent' => $parent,
			'id_parent' => $id_parent,
			'id_module_demo_author' => $id_module_demo_author
		);
		$this->db->where($data);
	 ...
	 *
	 */
	public function unlink_author_from_parent($parent, $id_parent, $id_module_demo_author)
	{
		$where = array(
			'parent' => $parent,
			'id_parent' => $id_parent,
